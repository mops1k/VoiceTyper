...
- **Выбор движка** — Whisper (по умолчанию) или NVIDIA Parakeet `parakeet-tdt-0.6b-v3`
  (мультиязычная, 25 европейских языков вкл. русский; точность уровня large при скорости
  small; офлайн-инференс на CPU через [parakeet.cpp](https://github.com/mudler/parakeet.cpp)).
...
Все модели — **q8-квантизация ggml** (Q8_0): заметно меньше по памяти и диску, чем fp16, при практически том же качестве и скорости.

| Размер | Файл на диске | Скорость | Качество | Комментарий |
|---|---|---|---|---|
| Tiny (q8) | ~42 МБ | очень быстро | низкое | для простых задач |
| Base (q8) | ~78 МБ | быстро | среднее | компромисс |
| **Small (по умолчанию)** (q8) | ~252 МБ | средне | высокое | хорошее качество RU/EN |
| Medium (q8) | ~785 МБ | медленно | очень высокое | точнее, медленнее |
| Large (turbo, q8) | ~834 МБ | очень медленно | максимальное | компактный, для мощных CPU |
...
При обновлении устаревшие fp16- и q5-файлы автоматически удаляются с диска.

### Модели Parakeet (GGUF)

Доступны при выборе движка **Parakeet** в разделе «Модели».
Модель `parakeet-tdt-0.6b-v3` (NVIDIA, CC-BY-4.0), кванты из
[mudler/parakeet-cpp-gguf](https://huggingface.co/mudler/parakeet-cpp-gguf):

| Квант | Файл на диске | Размер | Комментарий |
|---|---|---|---|
| q4_k | tdt-0.6b-v3-q4_k.gguf | ~644 МБ | минимум памяти |
| q5_k | tdt-0.6b-v3-q5_k.gguf | ~708 МБ | компромисс |
| q6_k | tdt-0.6b-v3-q6_k.gguf | ~775 МБ | выше точность |
| **q8_0 (по умолчанию)** | tdt-0.6b-v3-q8_0.gguf | ~897 МБ | лучшая точность, ~1 ГБ ОЗУ |

Parakeet v3 сама определяет язык (настройка языка для неё не требуется);
Whisper остаётся движком по умолчанию. Весовые файлы в поставку не входят —
один раз скачиваются с HuggingFace, далее распознавание полностью офлайн.
